...
type Service = { id: number; name: string; category: string; description: string; image_url: string; slug: string }
    ...
    <>
      ...
      <div style={{ marginTop: 68, background: '#15212c', padding: '88px 0 72px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 48px', position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,.58)', fontSize: 15, maxWidth: 560, lineHeight: 1.8 }}>
            Over 36 professional printing, branding, and design services — all under one roof.
          </p>
        </div>
      </div>
      ...
      <section style={{ padding: '56px 0 88px', background: '#f7f8fa' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 48px' }}>
          {filtered.length === 0 && (
            <p style={{ textAlign: 'center', color: '#737a80', padding: '60px 0' }}>No services found in this category yet.</p>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
            {filtered.map(svc => (
              <div key={svc.id} className="card-shadow" style={{ background: '#fff', overflow: 'hidden' }}>
                <div style={{ position: 'relative', height: 178 }}>
                  <Image src={svc.image_url || 'https://placehold.co/370x178'} alt={svc.name} fill style={{ objectFit: 'cover' }} sizes="33vw" />
                  <span style={{ position: 'absolute', top: 12, left: 12, background: '#ddb837', color: '#1a181d', fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '3px 10px' }}>{svc.category}</span>
                </div>
                <div style={{ padding: '20px 22px 24px' }}>
                  <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1a181d', marginBottom: 8 }}>{svc.name}</h3>
                  <p style={{ fontSize: 13, color: '#737a80', lineHeight: 1.7, marginBottom: 16 }}>{svc.description}</p>
                  <Link href="/quote" style={{ fontSize: 11, fontWeight: 700, color: '#ddb837', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                    Get a Quote →
                  </Link>
                </div>
              </div>
