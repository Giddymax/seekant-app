      ...
      <section style={{ padding: '72px 0 88px', background: '#fff' }}>
        <div className="service-detail-grid" style={{ maxWidth: 1120, margin: '0 auto', padding: '0 48px', display: 'grid', gridTemplateColumns: '1.35fr .65fr', gap: 48, alignItems: 'start' }}>
          <article>
            ...
            <h2 style={{ fontSize: 'clamp(24px,3vw,38px)', fontWeight: 900, color: 'var(--brand-heading, #1a181d)', letterSpacing: 0, lineHeight: 1.16, marginBottom: 18 }}>
              {content.headline}
            </h2>
            {content.paragraphs.map((paragraph, index) => (
              <p key={paragraph} style={{ color: 'var(--brand-text, #737a80)', fontSize: 15, lineHeight: 1.9, marginBottom: index === content.paragraphs.length - 1 ? 0 : 22 }}>
                {paragraph}
              </p>
            ))}
            ...
            <div className="service-copy-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 22, marginTop: 40 }}>
              <div style={{ background: '#f7f8fa', padding: '24px 24px 26px', borderTop: `3px solid ${color}` }}>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: 'var(--brand-heading, #1a181d)', marginBottom: 14 }}>What We Can Do</h3>
                <ul style={{ display: 'grid', gap: 10 }}>
                  {content.includes.map(item => (
                    <li key={item} style={{ display: 'flex', gap: 10, color: 'var(--brand-text, #737a80)', fontSize: 13, lineHeight: 1.7 }}>
              ...
              <div style={{ background: '#f7f8fa', padding: '24px 24px 26px', borderTop: `3px solid ${color}` }}>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: 'var(--brand-heading, #1a181d)', marginBottom: 14 }}>Best For</h3>
                <ul style={{ display: 'grid', gap: 10 }}>
          ...
          <aside style={{ background: '#f7f8fa', borderTop: `3px solid ${color}`, padding: '28px 28px 32px' }}>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: 'var(--brand-heading, #1a181d)', marginBottom: 14 }}>At a glance</h3>
            <div style={{ display: 'grid', gap: 14 }}>
              <div>
                <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#aaa', marginBottom: 4 }}>Category</p>
                <p style={{ fontSize: 14, color: 'var(--brand-heading, #1a181d)', fontWeight: 700 }}>{service.category}</p>
              </div>
              <div>
                <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#aaa', marginBottom: 4 }}>Availability</p>
                <p style={{ fontSize: 14, color: 'var(--brand-heading, #1a181d)', fontWeight: 700 }}>Available for quote</p>
              </div>
              <div>
                <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#aaa', marginBottom: 4 }}>Turnaround</p>
                <p style={{ fontSize: 14, color: 'var(--brand-heading, #1a181d)', fontWeight: 700, lineHeight: 1.6 }}>{content.turnaround}</p>
              </div>
              <div>
                ...
                <ul style={{ display: 'grid', gap: 8 }}>
                  {content.prepare.map(item => (
                    <li key={item} style={{ display: 'flex', gap: 9, color: 'var(--brand-text, #737a80)', fontSize: 13, lineHeight: 1.55 }}>
                      <ListMarker color={color} />
                      <span>{item}</span>
                    </li>
                  ))}
      ...
      {related && related.length > 0 && (
        <section style={{ padding: '64px 0 88px', background: '#f7f8fa' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 48px' }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: 'var(--brand-heading, #1a181d)', marginBottom: 32 }}>Related Services</h2>
              ...
              {related.map(item => (
                <Link key={item.id} href={`/services/${item.slug}`} className="card-shadow" style={{ background: '#fff', overflow: 'hidden', display: 'block' }}>
                  ...
                  <div style={{ padding: '18px 20px 22px' }}>
                    ...
                    <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--brand-heading, #1a181d)', lineHeight: 1.4 }}>{item.name}</h3>
                  </div>
                </Link>
              ))}
            </div>
